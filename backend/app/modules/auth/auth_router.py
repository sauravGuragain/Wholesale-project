...
# Cross-site cookie rules: when the frontend (e.g. Vercel) and backend (e.g.
# Render) live on different domains, the browser only keeps this cookie if it's
# SameSite=None + Secure. In local dev (same-origin via the Vite proxy) we use
# Lax without Secure so it works over plain http://localhost.
